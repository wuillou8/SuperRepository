...
function PrepareNews4Test( mynews::News, T1::DateTime, T2::DateTime, T3::DateTime )
    ...
    trainNews = News(filter(x -> (DateTime(x.date) < T2), mynews.news)) #+Day(2)), mynews.news))
    trainNews = News(filter(x -> (DateTime(x.date) >= T1), trainNews.news))
    testNews = News(filter(x -> (DateTime(x.date) >= T2), mynews.news))
    testNews = News(filter(x -> (DateTime(x.date) < T3), testNews.news))
    t2 = time()
...
# function extracting the entities from one New obj.
function GetEntities(new::New)
    # get Entities from new
    newEnts = String[]
...
function GetEntities(news::News)
    # get Entities from news
    newsEnts = String[]
    for new in unique(news.news)
        for ent in map(x->x["value"], new.entities)
            push!(newsEnts,ent)
        end
    end
...
# function extracting the entities from one New obj.
function GetEntities(mynews::News, guid::String)
    # get Entities from new
    new = getguid(mynews,guid)
    newsEnts = String[]
    for ent in map(x->x["value"], new.entities)
        push!(newsEnts,ent)
    end
...
# function extracting the entities from one New obj.
function GetEntities(mynews::News, guids::Array{String,1})
    # get Entities from new
    newsEnts = String[]
    for guid in guids
        new = getguid(mynews,guid)
...
# function extracting the entities from User
function GetEntities(totalnews::News, news::News, Usage::DataFrame) #, usedNews::Array{String,1})
    # get Entities from Usage
    usedEnts, newsEnts = String[], String[]
    for id in unique(Usage[:Custom1]) #Usage
        if isinguid(news,id)
            new = getguid(news,id)
            for ent in map(x->x["value"], new.entities)
                push!(usedEnts,ent)
...
function GetEntities(trainUsage::DataFrame, news::News)
    # get Entities from Usage
    t1 = time()
    usedEnts = String[]
...
# function correcting the statistics for the fact that news ids are not
...
# score recommendation with information retrieval approach
# usage Hash contains the userId's <==> {articlesId's}
function ScoreRecomm(testNews::News, usageHash::Dict{Any,Any}, recoList::Array{String,1})
    listNews = unique(map(x -> x.guid, testNews.news))

    __perf = Perfo(0,0,0,0)
    for userId in keys(usageHashDict_)
        usage = usageHashDict_[userId]
        [ add(__perf,perfoMeasOpt(listNews,recoList,use)) for use in usage ]
    end
    __perf
end
...
# score recommendation with information retrieval approach
# usage Hash contains the userId's <==> {articlesId's}
function ScoreRecomm(testNews::News, recoList::Array{String,1}, usage)
    listNews = unique(map(x -> x.guid, testNews.news))
    __perf = Perfo(0,0,0,0)
    [ add(__perf,perfoMeasOpt(listNews,recoList,use)) for use in usage ]
    __perf
...
# General function Scoring recommendations
function ScoreRecommendation(testNews::News, testUsage::DataFrame, recoList::Array{String,1})
    # preparatory step (filtering and speeding things up)
    listNews = unique(map(x -> x.guid, testNews.news))
    testUse = unique(testUsage[:AppUser_id])
    usageHashDict_ = { user =>  unique(@where(testUsage, :AppUser_id .== user)[:Custom1]) for user in testUse }

    # getting the performance looping over the users set.
    __perf = Perfo(0,0,0,0)
    for userId in keys(usageHashDict_)
        list = usageHashDict_[userId]

        # for each used item, a small experiment measure how precise the recommendation was.
        for lis in list
            addPerfo(__perf,perfoMeasOpt(listNews, recoList, lis))
        end
    end
...
function ScoreRecommendation(testNews::News, testUse::Array{String,1}, recoList::Array{String,1})
    # preparatory step (filtering and speeding things up)
    listNews = unique(map(x -> x.guid, testNews.news))
    #testUse = unique(testUsage[:AppUser_id])
    usageHashDict_ = { user =>  unique(@where(testUsage, :AppUser_id .== user)[:Custom1]) for user in testUse }
...
function ScoreRecommendation(testNews::News, recoList::Array{String,1}, usage)#userId::String)
    # preparatory step (filtering and speeding things up)
    listNews = unique(map(x -> x.guid, testNews.news))
    #usage =  unique(@where(testUsage, :AppUser_id .== userId)[:Custom1])
...
# function measuring information retrival metrics
# listnews: it the list of news
# listReco: the recommendation list
# idUser: used Id
# output: perf::Perfo
function perfoMeasOpt(listNews::Array{ASCIIString,1}, listReco::Array{String,1}, idUsed)
...
function EvaluatePerfo(testNews::News, testUsage::Dict{Any,Any}, scores::Array{Float64,1},  Nrec::Int64)
    ...
    idsNews = unique(map(x->x.guid,testNews.news))
    perf = Perfo(0,0,0,0)



    #usage = countmap( @where(testUsage, :AppUser_id .== userId)[:Custom1] ) #countmap(testUsage[:Custom1])
    usage = UTF8String[]
    for vals in values(usageHashDict_)
        #println(vals)
        for val in vals
            #println(val)
            push!(usage,val)
    ...
    perf #Precis(perf) , Recall(perf), Specif(perf), F1(perf)
end
