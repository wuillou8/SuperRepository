...
/** \class MapBase
  ...
  */
template<typename Derived, typename PScalar = typename ei_traits<Derived>::Scalar const * > class MapBase
...
//    using Base::operator=;
  ...
  /** Returns a pointer to the first coefficient of the matrix or vector.
    ...
    */
  inline const PScalar data() const
  {
    return m_data;
  }
  ...
  {
    ei_pstoret<Scalar, PacketScalar, StoreMode>
    (const_cast<PScalar>(m_data + index * innerStride()), x);
  }

  inline MapBase(const PScalar data) : m_data(data), m_rows(RowsAtCompileTime), m_cols(ColsAtCompileTime)
  {
    EIGEN_STATIC_ASSERT_FIXED_SIZE(Derived)
    checkSanity();
  }

  inline MapBase(const PScalar data, Index size)
    : m_data(data),
  ...
  inline MapBase(const PScalar data, Index rows, Index cols)
...
protected:
  ...
  void checkSanity() const
  {
    ...
    ei_assert(EIGEN_IMPLIES(ei_traits<Derived>::Flags&AlignedBit, (size_t((Scalar *)(&(m_data[0])))&0xf)==0)
              && "data is not aligned");
  }

  const PScalar EIGEN_RESTRICT m_data;
