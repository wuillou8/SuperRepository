...
struct ei_scalar_cast_op
{
  ...
  {
    return ei_cast<Scalar, NewType>(a);
  }
};
template<typename Scalar, typename NewType>
...
struct ei_scalar_real_ref_op
{
  ...
  EIGEN_STRONG_INLINE result_type& operator() (const Scalar& a) const
  {
    return ei_real_ref(*const_cast<Scalar*>(&a));
  }
};
template<typename Scalar>
...
struct ei_scalar_imag_ref_op
{
  ...
  EIGEN_STRONG_INLINE result_type& operator() (const Scalar& a) const
  {
    return ei_imag_ref(*const_cast<Scalar*>(&a));
  }
};
template<typename Scalar>
...
struct ei_scalar_constant_op
{
  ...
  {
    return ei_pset1<Packet>(m_other);
  }
  const Scalar m_other;
};
template<typename Scalar>
...
struct ei_linspaced_op_impl<Scalar,false>
{
  ...
  {
    return m_base = ei_padd(m_base,m_packetStep);
  }
...
template <typename Scalar, bool RandomAccess> struct ei_linspaced_op
{
  ...
  template<typename Index>
  EIGEN_STRONG_INLINE const Scalar operator() (Index i, Index = 0) const
  {
    return impl(i);
  }
  template<typename Index>
  EIGEN_STRONG_INLINE const Packet packetOp(Index i, Index = 0) const
  {
...
template<typename LhsScalar,typename RhsScalar> struct ei_functor_allows_mixing_real_and_complex<ei_scalar_product_op<LhsScalar,RhsScalar> >
{
  enum { ret = 1 };
};
